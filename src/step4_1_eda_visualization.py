...
PERF_PATH = Path("output/plots_step5_hybrid/model_results.csv")

OUTPUT_DIR = Path("output/eda_figures")
...
df = df[df[hardness_col].notnull()]
df = df[df[hardness_col] > 0].copy()
...
if perf_available:
    ...
    plt.bar(x - 0.2, perf["RMSE"], width=0.4)
    plt.bar(x + 0.2, perf["MAE"], width=0.4)
...
heatmap_df = numeric[top.tolist() + [hardness_col]].corr()
...
print("EDA figures saved to:", OUTPUT_DIR)
